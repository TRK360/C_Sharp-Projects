...
namespace Assignment_4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (Progress == "True")
            {
                Console.WriteLine("Sincerest apologies, however your package is too heavy to ship with Package Express.\n\n---Exiting program now.---");
                Environment.Exit(0);
            }
            else
            {
                ...
                int OADims = (Height + Width + Length);

                if (OADims > 50)
                {
                    Console.WriteLine("Sincerest apologies, however your package is too dimensionally eccentric to ship with Package Express.\n\n---Exiting program now.---");
                    Environment.Exit(0);
                }
                else { Console.WriteLine("\n\nYour estimate is now prepared:\n\n"); 
