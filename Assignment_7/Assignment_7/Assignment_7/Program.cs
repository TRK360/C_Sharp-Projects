...
//------------------------------------------------------------------------------------------------
            ...
            string[] array_one = new string[] { "one " + Appendage_1, "two " + Appendage_1 + "s", "three " + Appendage_1 + "s", "four " + Appendage_1 + "s", "five " + Appendage_1 + "s", "six " + Appendage_1 + "s", "seven " + Appendage_1 + "s", "eight " + Appendage_1 + "s", "nine " + Appendage_1 + "s", "ten " + Appendage_1 + "s" };//Manual Array
...
//------------------------------------------------------------------------------------------------
            ...
            Console.WriteLine("\n\nGuess which color is in my top 5 favorite list:\n");
            {
                ...
                int note = favColors.IndexOf(favcolor);//if index ocurrs as not found, this value returns -1 for later check below
                    ...
                    {
                        Console.WriteLine(String.Format("Element {0} is found at index {1}", favcolor, note));

                    }
                else
                {
                    Console.WriteLine("\n\nThat color is not one of my op 5 favories\n\n---CONTINUING PROGRAM---");
...
//------------------------------------------------------------------------------------------------
            ...
            { Console.WriteLine("Sincerest apologies, that color is not an option.\n\n---CLOSING PROGRAM NOW---");
                Environment.Exit(0);// exists program if color entered incorrectly
            }
 //------------------------------------------------------------------------------------------------
            ...
            foreach (string utensil in UtensilsList)//loops entire list
            {
                ...
                {
                    ResultantList.Add(utensil + " is a Duplicate Entity");//no matter what, something is getting added to the new list

                }

                else { ResultantList.Add(utensil); }//no matter what, something is getting added to the new list
